    ...
    try:
        ...
        for chat_id, shared_dict_id, is_admin in users_with_shared_dicts:
            try:
                ...
                print(f"SQLite error migrating user {chat_id}: {e}")
            except Exception as e:
                print(f"Unexpected error migrating user {chat_id}: {e}")
        
        conn.commit()
    except sqlite3.Error as e:
        print(f"SQLite error during migration_shared_dictionary_users: {e}")
        return False
    except Exception as e:
        print(f"Unexpected error during migration_shared_dictionary_users: {e}")
        return False
    finally:
        if conn:
            conn.close()
...
def fix_dictionary_admin_status():
    ...
    try:
        ...
        print(f"SQLite error during fix_dictionary_admin_status: {e}")
        return False
    except Exception as e:
        print(f"Unexpected error during fix_dictionary_admin_status: {e}")
        return False
    finally:
        if conn:
            conn.close()
